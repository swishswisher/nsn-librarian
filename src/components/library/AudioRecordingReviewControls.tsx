...
import type {
  AudioHumanLabel,
  AudioPrivacyState,
  BridgeAudioMetadataMutationResponse,
  BridgeAudioMetadataSummary,
  BridgeScannedFileSummary,
} from "@/lib/bridge/types";

type AudioRecordingReviewControlsProps = {
  file: BridgeScannedFileSummary;
};

const labelOptions: Array<{ label: string; value: AudioHumanLabel }> = [
...
const privacyOptions: Array<{ label: string; value: AudioPrivacyState }> = [
...
function labelsFrom(metadata: BridgeAudioMetadataSummary | null) {
  return new Set<AudioHumanLabel>(metadata?.humanLabels ?? []);
}

export function AudioRecordingReviewControls({
  file,
}: AudioRecordingReviewControlsProps) {
  const router = useRouter();
  const [audioMetadata, setAudioMetadata] = useState(file.audioMetadata);
  const [selectedLabels, setSelectedLabels] = useState(() =>
    labelsFrom(file.audioMetadata),
  );
  const [privacyState, setPrivacyState] = useState<AudioPrivacyState>(
    file.audioMetadata?.privacyState ?? "REVIEW_REQUIRED",
  );
  ...
  function toggleLabel(label: AudioHumanLabel) {
  ...
  async function saveAudioState(nextInput?: {
    ...
    try {
      const response = await fetch(
        `/api/bridge/scanned-files/${encodeURIComponent(file.id)}/audio`,
        {
          body: JSON.stringify({
            labels: nextLabels,
      ...
      const payload =
        (await response.json()) as BridgeAudioMetadataMutationResponse;
      ...
      setAudioMetadata(payload.file.audioMetadata);
      setSelectedLabels(labelsFrom(payload.file.audioMetadata));
      setPrivacyState(
        payload.file.audioMetadata?.privacyState ?? "REVIEW_REQUIRED",
      );
      setMessage("Recording review details were saved.");
      router.refresh();
    } catch {
      setError("The Librarian could not save these recording details.");
    } finally {
      setIsSaving(false);
    }
  }
    ...
    <div className="grid gap-4">
      ...
      <label className="grid gap-2 text-sm font-semibold text-[var(--nsn-navy)]">
        ...
        <select
          ...
          onChange={(event) =>
            setPrivacyState(event.target.value as AudioPrivacyState)
          }
          value={privacyState}
        >
      ...
      {audioMetadata ? (
        <p className="break-words text-sm leading-6 text-[var(--nsn-slate)] [overflow-wrap:anywhere]">
          Current review state: {labelText(audioMetadata.privacyState)}
          {audioMetadata.humanLabels.length > 0
            ? ` - ${audioMetadata.humanLabels.map(labelText).join(", ")}`
            : ""}
        </p>
      ) : null}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        <NsnButton
          disabled={isSaving}
          onClick={() => saveAudioState()}
          type="button"
          variant="primary"
        >
          {isSaving ? "Saving..." : "Save Recording Labels"}
        </NsnButton>
        <NsnButton
          disabled={isSaving}
          onClick={keepPrivate}
